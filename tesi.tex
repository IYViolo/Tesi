...
Moreover
\begin{itemize}
\item if $\phi_j \in C^m(\overline W_i)$ with  $\| D^{\alpha}\phi \|\le M <\infty $, for every $1\le|\alpha|\le m$, we say that $\Omega$  has a resolved $C^m$ boundary with parameters $d,D, \varkappa,M$.
...
Let $l,n \in \mathbb{N}, 1\le p \le \infty$ and $W = \prod\limits_{i=1}^{n-1}]a_i,b_i[$ be an open cuboid of $\mathbb{R}^{n-1}$. Moreover define
\[ S=W \times \mathbb{R} \]
\[\Omega =\{ (\overline x,x_n) | \overline x\in W, x_n<\phi(\overline x)  \} \]
where $\phi \in C^m(\overline W)$,$m\ge l$, and $\| D^{\alpha}\phi \|\le M <\infty $ for every $1\le|\alpha|\le l$. Then for $0\le \gamma \le n$ and $f \in W^{l,p}(\Omega)$ 
\begin{enumerate}[i)]
\item  $\| Tf\|_{M_p^\gamma(S)} \le    C\| f\|_{M_p^\gamma(\Omega)}$
\item $\| D^\alpha_w Tf\|_{M_p^\gamma(S)} \le   C\sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wf\|_{M_p^\gamma(\Omega)}$,  $1\le |\alpha|\le l$
\end{enumerate}
where $T: W^{l,p}(\Omega) \rightarrow W^{l,p}(S)$ is the Hestenes operator defined in Lemma $\ref{hestenes1}$ and $C$ is a constant independent of $f$.
\end{lemma}
...
\[ I \le \underbrace{\left ( \frac{1}{r^\gamma} \int_{B_r(x_0)\cap \Omega^+} |D^\alpha_w Tf(x)|^pdx \right)^{\frac{1}{p}}}_\textrm{$I_1$}+\underbrace{\left ( \frac{1}{r^\gamma} \int_{B_r(x_0)\cap \Omega} |D^\alpha_w Tf(x)|^pdx \right)^{\frac{1}{p}}}_\textrm{$I_2$}.\]
Since $Tf(x)=f(x)$ when $ x\in\Omega$, we have immediately  
\[ I_2 \le \|D^\alpha_w f \|_{M_p^\gamma(\Omega)} . \]
...
By the previous inequality and Lemma \ref{circle} we are able to produce the following bound
\begin{align*}
 \frac{\|D^{\alpha}_w(f \circ G_k) \|_{L^p(B_r(x_0)\cap \Omega^+)}}{\phi(r)^{\frac{1}{p}}} &\le C_k \sum_{1\le |\beta|\le|\alpha| } \left(\phi(r)^{-1} \int_{G_k(B_r(x_0)\cap \Omega^+)} |D^\alpha_wf(y)|^p|\det JG_k^{-1}\big|_{G_k(y)}|dy\right)^{\frac{1}{p}}\\
 		&\le C_k \beta_k^{-\frac{1}{p}} \sum_{1\le |\beta|\le|\alpha| } \left(\phi(r)^{-1} \int_{B_{c_kr}(A_{\beta_k}(x_0))\cap \Omega} |D^\alpha_wf(y)|^pdy\right)^{\frac{1}{p}} 		\\		
\end{align*}
where $c_k$ depends only on $\beta_k$ and $M$ and $A_{\alpha_k}$ is defined as in Lemma \ref{circle}. Next we estimate $I_1$:

\begin{align*}
I_1 = r^{-\frac{\gamma}{p}}\|D^\alpha_w Tf \|_{L^p(B_r(x_0)\cap \Omega^+)}&\le r^{-\frac{\gamma}{p}} \sum_{k=1}^l \alpha_k \|D^\alpha_w f(G_k) \|_{L^p(B_r(x_0)\cap \Omega^+)} \\
																		&\le \sum_{k=1}^l \alpha_kC_k\beta_kc_k^{\frac{\gamma}{p}} \left( \sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wf\|_{M_p^\gamma(\Omega)} \right).																	
\end{align*}
Finally putting the estimates of $I_1,I_2$ together 
\begin{align*}
\| D^\alpha_w Tf\|_{M_p^\gamma(S)}&=\sup_{x_0 \in S, r>0} \left ( \frac{1}{r^\gamma} \int_{B_r(x_0)\cap S} |D^\alpha_w Tf(x)|^pdx \right)^{\frac{1}{p}} \\
									&\le \|D^\alpha_w f \|_{M_p^\gamma(\Omega)}+\sum_{k=1}^l \alpha_kC_k\beta_kc_k^{\frac{\gamma}{p}} \left( \sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wf\|_{M_p^\gamma(\Omega)} \right) \\
									& \le \widetilde C\sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wf\|_{M_p^\gamma(\Omega)}
\end{align*}
where $\widetilde C$ depends only on $\{b_k\}_k,\{\alpha_k\}_k,l,M,\gamma,p$. This proves ii). The proof of i) is exactly analogous to the proof of ii).
\end{proof}
		

\begin{theorem}
Let $m,l \in \mathbb{N}, l\le m$, $1\le p\le\infty$ and $\Omega$ a domain in $\mathbb{R}^n$ with a $C^m$ resolved boundary (with parameters $d,D,M, \varkappa$). Then for $0\le \gamma \le n$ and $f \in W^{l,p}(\Omega)$ 
\begin{enumerate}[i)]
\item  $\| Tf\|_{M_p^\gamma(\rr^n)} \le    C\| f\|_{M_p^\gamma(\Omega)}$
\item $\| D^\alpha_w Tf\|_{M_p^\gamma(\rr^n)} \le   C\sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wf\|_{M_p^\gamma(\Omega)}$,  $1\le |\alpha|\le l$
\end{enumerate}
where $ T: W^{l,p}(\Omega) \rightarrow W^{l,p}(\mathbb{R}^n)$ is the Hestenes operator define in Theorem \ref{teorh} and $C$ doesn't depend on $f$.
\end{theorem}
\begin{proof}
...
where the operator $\widetilde T_i: \widehat W^{l,p}(V_i\cap \Omega) \rightarrow W^{l,p}(V_i)$ as $\widetilde T_i f = T_i(f(\lambda_i^{-1}))(\lambda_i)$. 
Before starting the proof we remark some facts that will be justified at the end of the proof:
\begin{enumerate}[a)]

\item Let $C_i$ the constant relative to $T_i$ such that
\[ \| T_ig\|_{M_p^\gamma(\lambda_i(V_i))}\le C_i \|g\|_{M_p^\gamma(\lambda_i(\Omega \cap V_i))}\]
\[\| D^\alpha_w T_ig\|_{M_p^\gamma(\lambda_i(V_i)))} \le   C_i\sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wg\|_{M_p^\gamma(\lambda_i(\Omega \cap V_i)))}\]  

for $1\le |\alpha|\le l$ and $g \in \widehat W^{l,p}(\lambda_i(\Omega \cap V_i))$. Then $\sup_{i=1,...,s} C_i \le M_3$, where $M_2$ doesn't depend on $i$.
\item We have
\[ \| \widetilde T_ig\|_{M_p^\gamma(V_i)}\le C_i \|g\|_{M_p^\gamma(\Omega \cap V_i)}\]
\[\| D^\alpha_w \widetilde T_ig\|_{M_p^\gamma(V_i)} \le   M_3C_i\sum_{1\le |\beta|\le|\alpha| }\| D^\alpha_wg\|_{M_p^\gamma(\Omega \cap V_i)}\]  
for $1\le |\alpha|\le l$ and $g \in \widehat W^{l,p}(\Omega \cap V_i)$ and where $M_3$ doesn't depend on $i$. 
\item If $X= \bigcup_k X_k$ is a countable union of measurable sets $X_k$ and the multiplicity of the covering is less than $\delta$, then for every function measurable on $X$
\[ \left ( \sum_k \| f\|^p_{M^\gamma_p(X_k)} \right )^{\frac{1}{p}} \le \delta^{\frac{1}{p}}\| f\|_{M^\gamma_p(X)}. \] 
...
\begin{align*}
\|Tf \|_{M_p^\gamma(\rr^n)} &\overset{e)}\le  \varkappa^{\frac{p-1}{p}} \left ( \sum_{i=1}^s \|\psi_i\widetilde T_i(\psi_if)\|_{M^\gamma_p(\rr^n)}^p \right)^{\frac{1}{p}} \\
							&\le  \varkappa^{\frac{p-1}{p}} \left ( \sum_{i=1}^s \|\widetilde T_i(\psi_if)\|_{M^\gamma_p(V_i)}^p \right)^{\frac{1}{p}} && \text{because } \supp \psi_i \subset V_i,|\psi_i|\le1\\
							&\overset{a)-b)}\le \varkappa^{\frac{p-1}{p}} M_2\left ( \sum_{i=1}^s \|\psi_if\|_{M^\gamma_p(V_i\cap\Omega)}^p \right)^{\frac{1}{p}}&& \\
							&\le  \varkappa^{\frac{p-1}{p}} M_2\left ( \sum_{i=1}^s \|f\|_{M^\gamma_p(V_i\cap\Omega)}^p \right)^{\frac{1}{p}}\\
							&\overset{c)}\le  \varkappa M_2\|f\|_{M^\gamma_p(\Omega)}.
\end{align*}
...
As in the proof of i) we divide both sides by $r^{\frac{\gamma}{p}}$ and apply the supremum
\begin{align*}
\|D^\alpha_w Tf \|_{M_p^\gamma(\rr^n)} &\overset{e)} \le  \varkappa^{\frac{p-1}{p}} C_\alpha M_1\left ( \sum_{\beta\le \alpha}\sum_{i=1}^s \|D^\beta_w (\widetilde T_i(\psi_if))\|_{M^\gamma_p(\rr^n)}^p \right)^{\frac{1}{p}} \\
										&=\varkappa^{\frac{p-1}{p}} C_\alpha M_1\left ( \sum_{\beta\le \alpha}\sum_{i=1}^s \|D^\beta_w (\widetilde T_i(\psi_if))\|_{M^\gamma_p(V_i)}^p \right)^{\frac{1}{p}}\\
										&\overset{a)-b)} \le \varkappa^{\frac{p-1}{p}} C_\alpha M_1M_2M_3\left ( \sum_{\beta\le \alpha}\sum_{i=1}^s \sum_{|\gamma|\le |\beta|}\|D^\gamma_w (\psi_if)\|_{M^\gamma_p(V_i\cap \Omega)}^p \right)^{\frac{1}{p}}\\
										&\le \varkappa^{\frac{p-1}{p}} \widetilde C_\alpha M_1M_2M_3\left ( \sum_{|\beta|\le |\alpha|}\sum_{i=1}^s\|D^\beta_w (\psi_if)\|_{M^\gamma_p(V_i\cap \Omega)}^p \right)^{\frac{1}{p}}\\
										&\overset{c)}\le \varkappa \widetilde C_\alpha M_1M_2M_3 \sum_{|\beta|\le |\alpha|} \|D^\beta_w (\psi_if)\|_{M^\gamma_p(\Omega)} \\
										&\le \varkappa \widetilde C_\alpha M_1M_2M_3 \sum_{|\beta|\le |\alpha|}C_\beta \sum_{\gamma\le \beta} \|D^\beta_w f\|_{M^\gamma_p(\Omega)} && \text{by product rule} \\
										&\le \varkappa \widetilde C'_\alpha M_1M_2M_3 \sum_{|\beta|\le |\alpha|}\|D^\beta_w f\|_{M^\gamma_p(\Omega)}.
\end{align*}
