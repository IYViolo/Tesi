...
Let $\Omega$ be a domain in $\rr^n$ and suppose that there exists a rigid rotation $R$ of $\rr^n$ such that $R(\Omega)$ is a special Lipschitz domain. We observe that all the results stated above that holds for a special Lipschitz domain, holds also for $\Omega.$ This follows directly after a suitable change of coordinates in $\rr^n$. 
\end{remark}

\begin{definition}\label{minsmooth}
Let $\Omega$ be an open set in $\rr^n$ and let $\partial \Omega$ be its boundary. We say that $\partial \Omega$ is minimally smooth if there exists an $\varepsilon >0$, an integer $N$, an $M>0$ and a sequence $\{U_i\}_{i=1}^s$ (where $s$ can be $+\infty$) of open sets such that:
\begin{enumerate}[i)]
...
\item No point of $\rr^n$ is contained in more than $N$ of the $U_i$'s.
\item For every $i$ there exist a special Lipschitz domain $D_i$ and a rigid rotation $R_i$ of $\rr^n$ such that
\[ U_i\cap \Omega = U_i \cap R_i(D_i).\]
\item The Lipschitz bound of $D_i$ does not exceed $M$ for every $i$. 
\end{enumerate}
\end{definition}

We now give the outline of the construction of the Stein extension operator for a set with minimally smooth boundary. The details of this construction can also be found in \cite{stein}. 

First we introduce the following notation: given a set $U$ in $\rr^n$ and an $\varepsilon >0$ we denote by $U_\varepsilon=\{ x 	\in U \ | \ B_\varepsilon(x) \subset U\}$. Now let $\Omega$ be an open set in $\rr^n$ with minimally smooth boundary $\partial \Omega$. Consider also the constants $\varepsilon,N,M$ and the sequence of open sets  $\{U_i\}_{i=1}^s$ relative to $\Omega$ as given in Definition \ref{minsmooth}. We can construct a sequence of real-valued functions $\{\lambda_i\}_{i=1}^s$ defined in $\rr^n$, such that
\begin{itemize}
...
\begin{itemize}
	\item $\supp \Lambda_+ \subset \Omega_\varepsilon \cup \{ x \in \rr^n \ | \ d(x,\partial \Omega)\le \varepsilon /2\},$
...
\end{itemize}
Consider now the extension operators $S_i$ for $W^{l,p}(R_i(D_i))$, that is well defined by Remark \ref{rotlip} and Theorem \ref{Sdef}. We define the extension operator $E$ for $\Omega$ as follows
\begin{equation} 
Ef(x):= \Lambda_+(x)\frac{\sum_{i=1}^\infty \lambda_i(x)S_i(\lambda_if)(x)}{\sum_{i=1}^\infty \lambda^2_i(x)}+\Lambda_-(x)f(x). \label{defEE}
\end{equation}
...
\[ \|f\|_{M_{p,Q}^\phi(\Omega)}:=\sup_{\substack{Q_{2r}(x) \\ x \in \Omega \\r>0}} \left(  \frac{1}{\phi(r)}\int_{Q_{2r}(x)\cap \Omega} |f(y)|^p dy \right )^{\frac{1}{p}}\]
...
\begin{equation}
\left(\frac{1}{\phi(r/2)}\int_Q |Tf(x)|^pdx \right)^{\frac{1}{p}} \le C_n(M) \| f\|_{M_{p,Q}^\phi(\Omega)} \label{cubestimate}
\end{equation}
...
\[\left(\frac{1}{\phi(r/2)}\int_Q|Tf(x)|^p dx\right)^{\frac{1}{p}} \le \frac{A_3c^2m}{2} \sum_{k=0}^\infty s_k(\alpha) \| f\|_{M_{p,Q}(\Omega)} \]
...
Hence we get 
\[\left(\frac{1}{\phi(r/2)}\int_Q|Tf(x)|^p dx\right)^{\frac{1}{p}} \le \frac{3mA_3c^2}{2} \| f\|_{M_{p,Q}^\phi(\Omega)} \]
...
\begin{equation}
\frac{1}{\phi(r/2)^{\frac{1}{p}}} \| Tf\|_{L^p(Q^-_1)} \le c_1 \| f\|_{M_p^\phi(\Omega)} \label{q1}
\end{equation}
...
\[  \left(\frac{1}{\phi(r/2)}\int_{S_2} \int_{\psi(\overline x)-\delta-r}^{\psi(\overline x)-\delta}|Tf(\overline x,y)|^p dy d\overline x\right)^{\frac{1}{p}}\le c_2 \| f\|_{M_p^\phi(\Omega)} \]
